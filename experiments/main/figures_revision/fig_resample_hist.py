...
def main(
    ...
    if len(outputs_files) != 4:
        print("Not all results have completed generation, some seeds appear missing.")
        exit()
    outputs_by_instance = {}
    instances = set()
    for outputs_file in outputs_files:
        ...
        if len(outputs) != 159:
            print(
                f"File {outputs_file} has {len(outputs)} outputs, not 159, likely did not finish generation yet!."
            )
            exit()
